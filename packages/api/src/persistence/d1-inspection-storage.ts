...
export class D1InspectionStorage implements InspectionStorage {
  ...
  async readUserById(id: string): Promise<UserRecord | null> {
    const row = await this.selectFirst(`SELECT * FROM "users" WHERE "id" = ?`, id);
  ...
  async resolveComment(mutation: ResolveCommentStorageMutation): Promise<void> {
    const statements: D1PreparedStatement[] = [
      this.db
        .prepare(
          `UPDATE "comments"
           SET "status" = ?, "closedBy" = ?, "closedAt" = ?, "closedInRoundId" = ?, "resolveRemark" = ?, "updatedAt" = ?
           WHERE "id" = ?`
        )
        .bind(
...
function mapCommentRecord(row: JsonRow): CommentRecord {
  return {
    ...
    resolveRemark: nullableStringValue(row.resolveRemark),
    createdAt: stringValue(row.createdAt),
    updatedAt: stringValue(row.updatedAt)
  };
}
