...
| Auth / RBAC | 🟡 | Backend complete: JWT/middleware/role checks; inspections routes require bearer auth; /api/auth/me returns profile. Frontend now has real login, token persistence, bearer API injection, protected routing, logout UX, post-login redirect, and session-expired redirect; broader session lifecycle / RBAC polish is still pending. |
| Comment resolve | ✅ | Backend route + repository + tests done; Frontend Dashboard UI now resolves comments via the API and refreshes local detail/list state after each resolve |
...
- Broader frontend auth/RBAC polish beyond the current protected-shell flow (for example richer role-aware navigation and longer-lived session lifecycle decisions).
- Token refresh / invalidation strategy beyond the current logout + session-expired redirect behavior.
- Wider application of project-scope authorization beyond the currently protected inspection routes.
...
- Auth now supports JWT issuance + verification, protects the inspection API routes, enforces project-scoped read filtering on inspection list/detail, and has a real frontend login/session shell in place. The next increment should deepen RBAC/session polish instead of redoing the initial wiring.
...
- `✅ Complete for MVP baseline`: shared contracts, core domain rules, multi-page routing, Dashboard UI, Project Hall, Login Portal, protected shell routing, and dashboard comment resolve UI.
- `🟡 Partial / in progress`: persistence architecture hardening, D1 rollout, deeper RBAC/session polish, per-item report triggers, cross-module data syncing.
- `❌ Not started in product code`: complex import pipeline automation and server-side PDF rendering.

## Practical Next Priorities

1. **[P1]** Deepen frontend auth/RBAC polish beyond the current protected-shell flow (for example role-aware navigation, guard coverage for remaining pages, and session UX hardening).
2. **[P1]** Build Admin page capabilities on top of the existing shell (user/project/ship management).
3. **[P2]** Build Reports page (statistics and trends).
4. **[P2]** Exercise the D1-backed route path in a real Worker/D1 environment.
5. **[P2]** Narrow the D1 write path for `resolveComment`.
6. **[P3]** Build automated import / PDF / n8n delivery flows.
